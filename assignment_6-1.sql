...
# cannot use USING if fk key and primary key do not have the same name
# option 2
SELECT invoice_id, SUM(price * quantity) AS subtotal, SUM((price * quantity) * .075) AS tax, SUM(price * quantity) + SUM(price * quantity) * .075, SUM(price * quantity) * 1.075 AS total
FROM invoice
INNER JOIN invoice_item USING (invoice_id)
INNER JOIN item USING (item_id)
GROUP BY invoice_id 
